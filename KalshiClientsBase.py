...
class ExchangeClient(KalshiClient):
    ...
    def get_series(self):
        dictr = self.get("/v1/series")
        return dictr
    
    def get_ranged_market_by_ticker(self, ticker):
        dictr = self.get(f"/v1/ranged_markets_by_ticker/{ticker}")
        return dictr
    
    def get_events_cached(self):
        dictr = self.get(f"/v1/events")
        return dictr